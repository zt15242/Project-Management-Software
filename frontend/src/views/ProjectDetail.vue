      ...
      <el-tab-pane label="知识库" name="knowledge">
        <el-card>
          <template #header>
            ...
            >
              <span>知识库文档</span>
              <el-button type="primary" @click="showUploadKnowledgeDialog">
          ...
          >
            ...
            <el-table-column label="浏览" width="80">
              <template #default="{ row }">
                <el-icon><View /></el-icon> {{ row.views }}
              </template>
            </el-table-column>
          ...
          <el-empty
            v-if="knowledgeList.length === 0 && !knowledgeLoading"
            description="暂无文档"
          />

          <!-- 统计信息 -->
          <el-row :gutter="20" style="margin-top: 20px" v-if="knowledgeStats">
            <el-col :span="8">
              <el-statistic
                title="文档总数"
                :value="knowledgeStats.total_docs"
              />
            </el-col>
            <el-col :span="8">
              <el-statistic
                title="总浏览量"
                :value="knowledgeStats.total_views"
              />
            </el-col>
            <el-col :span="8">
              ...
              <div>
                <el-tag
                  v-for="(count, type) in knowledgeStats.file_type_stats"
                  :key="type"
                  size="small"
            ...
            <div class="sessions-panel">
              ...
              <div class="session-list">
                ...
                >
                  ...
                  >
                    <el-icon class="session-more-btn" @click.stop
                      ><MoreFilled
                    /></el-icon>
                    <template #dropdown>
                      <el-dropdown-menu>
            ...
            <div class="chat-panel">
              ...
              <div class="chat-messages" ref="chatMessagesRef">
                <div v-if="chatMessages.length === 0" class="chat-welcome">
                  ...
                  <h3>我是您的AI项目助手</h3>
                  <p>
                    您可以勾选左侧的资源，然后问我相关问题，或者让我帮您生成文档。
                  </p>
                </div>
                  ...
                  <div class="message-content-wrapper">
                    ...
                    <div class="message-bubble-container">
                      <div class="message-bubble">
                        ...
                        ></div>
                        <div v-else-if="editingMessageId !== message.id">
                          {{ message.content }}
                        </div>
                        ...
                        >
                          ...
                          <div class="edit-footer">
                            <el-button size="small" link @click="cancelEdit"
                              >取消</el-button
                            >
                            <el-button
                              size="small"
                              type="primary"
                              @click="saveEdit(message, index)"
                              >保存并重新生成</el-button
                            >
                          </div>
                        </div>

                        <!-- 智能 PPT 识别按钮 (仅 Assistant) -->
                        <div
                          v-if="
                            message.role === 'assistant' &&
                            isPPTOutline(message.content)
                          "
                        ...
                        >
                          <el-button
                            ...
                            size="small"
                            plain
                            @click="parseAndOpenPPT(message.content)"
                          >
                            <el-icon><Monitor /></el-icon> 识别到大纲，立即进入
                            PPT 创作
                          </el-button>
                        </div>
                        ...
                        <div
                          ...
                          class="tool-calls-result"
                        >
                          <div
                            v-for="(tool, tIndex) in message.tool_calls"
                            :key="tIndex"
                            class="tool-wrapper"
                          >
                            <!-- 任务建议特殊展示 -->
                            <div
                              v-if="getToolName(tool) === 'propose_tasks'"
                              class="task-proposal-card"
                            >
                              <div class="proposal-header">
                                <span class="proposal-title">
                                  <el-icon><List /></el-icon> 识别到
                                  {{ getToolArgs(tool)?.tasks?.length || 0 }}
                                  个待办任务
                                </span>
                                <el-button
                                  type="primary"
                                  size="small"
                                  @click="confirmCreateTasks(tool)"
                                  :loading="
                                    creatingTasks &&
                                    currentcreatingTool === tool
                                  "
                                  :disabled="tool.created"
                                >
                                  {{ tool.created ? "已创建" : "一键创建" }}
                                </el-button>
                              </div>
                              <div class="proposal-list">
                                <div
                                  v-for="(task, idx) in getToolArgs(tool)
                                    ?.tasks || []"
                                  :key="idx"
                                  class="proposal-item"
                                >
                            ...
                            <div
                              v-if="
                                getToolName(tool) === 'generate_ppt_outline'
                              "
                              class="ppt-outline-proposal-card"
                            >
                              <div class="proposal-header">
                                <span class="proposal-title">
                                  <el-icon><Monitor /></el-icon>
                                  已生成 PPT 初稿大纲：{{
                                    getToolArgs(tool)?.title
                                  }}
                                </span>
                                <el-button
                                  type="primary"
                                  size="small"
                                  @click="openPPTCreator(tool)"
                                >
                                  编辑并生成 PPT
                                </el-button>
                              </div>
                              <div class="ppt-outline-preview">
                                <el-timeline>
                                  <el-timeline-item
                                    v-for="(slide, sIdx) in getToolArgs(
                                      tool
                                    )?.slides?.slice(0, 3)"
                                    :key="sIdx"
                                    :title="slide.title"
                                  ...
                                  >
                                    <div class="slide-mini-item">
                                      <strong
                                        ...
                                        {{ slide.title }}</strong
                                      >
                                      <div class="slide-mini-content">
                                        {{ slide.content[0] }}...
                                      </div>
                                    </div>
                                  </el-timeline-item>
                                  <el-timeline-item
                                    v-if="getToolArgs(tool)?.slides?.length > 3"
                                    hollow
                                  >
                                    ... 还有
                                    {{ getToolArgs(tool).slides.length - 3 }} 页
                                  </el-timeline-item>
                                </el-timeline>
                              </div>
                            </div>
                      ...
                      <!-- 消息操作按钮 (悬浮显示) -->
                      <div
                        class="message-item-utils"
                        v-if="!aiThinking && !message.is_streaming"
                      >
                        <el-tooltip
                          content="编辑并重发"
                          placement="top"
                        ...
                        >
                          <el-button
                            size="small"
                            link
                            @click="regenerate(message, index)"
                          >
                ...
                <!-- message-row -->
                <div v-if="aiThinking" class="message-wrapper assistant">
                  <div class="message-avatar">
                    <el-avatar
                      :size="36"
                      style="
                  ...
                  <div class="message-content">
                    <div class="thinking-indicator">
                      ...
                      <span v-if="currentActiveSkill === 'research'"
                        >正在运用 <b>深度研究</b> 超能力检索信息...</span
                      >
                      <span v-else-if="currentActiveSkill === 'ppt_design'"
                        >正在运用 <b>PPT设计</b> 超能力构思大纲...</span
                      >
              ...
              <!-- 已选资源显示 -->
              <div
                class="selected-resources-bar"
                @click="resourceDrawerVisible = true"
              >
                <div class="selected-count" v-if="selectedResourcesCount > 0">
                  <el-icon><FolderChecked /></el-icon>
                  <span>已引用 {{ selectedResourcesCount }} 个资源</span>
                  <el-link
                    type="primary"
                    :underline="false"
                ...
                <div
                  class="selected-count placeholder"
                  v-else
                  style="color: #909399"
                >
                  <el-icon><Plus /></el-icon>
                  <span>点击选择知识库文档或会议纪要作为上下文</span>
                </div>

                <el-button
                  type="text"
                  @click.stop="clearSelectedResources"
              ...
              <div class="chat-input-area">
                <el-input
                  ...
                  type="textarea"
                  :rows="3"
                  placeholder="输入您的问题，AI将基于选中的知识库和会议纪要为您解答..."
                ...
                <div class="input-actions">
                  <div class="tool-buttons">
                    <el-dropdown @command="handleToolCommand">
                      ...
                      <template #dropdown>
                        <el-dropdown-menu>
                          <el-dropdown-item command="generate-doc">
                          ...
                          <el-dropdown-item command="extract-tasks">
                  ...
                  <el-tab-pane label="知识库" name="knowledge">
                    <div class="drawer-resource-list">
                      ...
                      <div class="resource-grid">
                        <div
                          v-for="doc in filteredKnowledgeList"
                          :key="doc.id"
                          class="drawer-resource-item"
                          :class="{
                            selected: selectedResources.knowledge.includes(
                              doc.id
                            ),
                          }"
                          @click="toggleResource('knowledge', doc.id, doc)"
                        >
                          <div class="item-icon">
                            {{ getFileIcon(doc.file_type) }}
                          </div>
                          <div class="item-info">
                            <div class="item-title" :title="doc.title">
                              {{ doc.title }}
                            </div>
                            <div class="item-meta">
                              {{ doc.category }} ·
                          ...
                          <div class="item-check">
                            <el-checkbox
                              :model-value="
                                selectedResources.knowledge.includes(doc.id)
                              "
                    ...
                    <div class="drawer-resource-list">
                      <div class="resource-grid">
                        <div
                          ...
                          class="drawer-resource-item"
                          :class="{
                            selected: selectedResources.meetings.includes(
                              meeting.id
                            ),
                          }"
                          @click="
                            toggleResource('meetings', meeting.id, meeting)
                          "
                        >
                          <div class="item-icon">
                            {{ meeting.file_type === "video" ? "🎥" : "🎙️" }}
                          </div>
                          <div class="item-info">
                            <div class="item-title" :title="meeting.title">
                              {{ meeting.title }}
                            </div>
                            <div class="item-meta">
                          ...
                          <div class="item-check">
                            <el-checkbox
                              :model-value="
                                selectedResources.meetings.includes(meeting.id)
                              "
                              style="pointer-events: none"
                            />
                          </div>
